...
import Description from './component/Description' // 説明コンポーネントをインポート
...
const Login: React.FC = () => {
  ...
  const [showDescription, setShowDescription] = useState(false) // 説明表示の状態を管理
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-pink-100 font-sans">
        ...
        <div className="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-8 rounded-lg shadow-lg text-center">
            ...
            <button
              ...
              className="px-4 py-2 bg-pink-100 text-white rounded-lg hover:bg-pink-200"
            >
              OK
            </button>
          </div>
      ...
      {showDescription && (
        <Description onClose={() => setShowDescription(false)} />
      )}
        ...
        {loading ? (
          ...
          <>
            <img
              src={logo}
              alt="Logo"
              className="mx-auto mb-24 w-48 h-48 sm:w-72 sm:h-72 lg:w-96 lg:h-96"
            />
            <p className="text-lg pb-4 text-pink-400">
              ～おかし好きのためのかわいい記録帳♡～
            </p>
            <p
              className="border-b-2 border-gray-300 mb-4 text-gray-300 cursor-pointer"
            ...
            <button
              onClick={() => {
                ...
                if (userAgent.includes('line')) {
                  alert(
                    'LINE内ブラウザではGoogleログインがサポートされていません。外部ブラウザで開いてください。',
                  )
                } else {
                  signInWithGoogle()
                }
